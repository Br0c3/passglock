...
import encode
import file
#from passmanage.file import File
...
def init():
    ...
    fic: File = file.File(fichier, key)
...
def save():
    ...
    # le nom du fichier
    fichier = input("\n\033[33mEntrez le nom du fichier avec l'extantion json :: \033[0m")
    # vérifier si le nom entré correspond avec le format attendu
    if not Fic.fullmatch(fichier):
        # relever une erreur
        raise AssertionError
...
def add(fichier: file.File):
    ...
    data = [fichier.f_compt()]  # numéro d'index de notre dernière sauvegarde au premier indice
    # de la nouvelle liste data.
...
def mod(fichier: file.File):
    ...
    for i in ha:  # parcourir la liste de données
        if i == "Mot de passe":  # si la donnée demander est le mot de passe,
            ...
            da = input(f"\n\033[33mEntrez [{i}] :: \033[0m")
            # si numéro d'index de notre dernière sauvegarde est inferieur a l'index entrer ou
            # l'index entrer est négatif.
...
def delet(fichier: file.File):
    index = input(f"\n\033[33mEntrez l'index de la donnée à supprimer:: \033[0m")
    fichier.d_del(index)
    fichier.f_list()
...
def scd(fichier: file.File):
    while True:  # boucle infini
        ...
        dict = {"1": add, "2": mod, "3": aff, "4": delet, "5": prc, 6: exit}
        if choix in "1234":  # les fonctions ayants besoin de paramettre
            dict[choix](fichier)
        elif choix in "56":  # les autres fonctions
            dict[choix]()
        else:
